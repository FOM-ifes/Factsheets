...
\begin{itemize}
  \item für \texttt{x1}: $\Sexpr{round(KIs["x1", 1], 4)}$ bis $\Sexpr{round(KIs["x1", 2], 4)}$ den wahren, unbekannten Wert der Steigung von \texttt{x1} in der Population;
  \item für \texttt{x2B}: $\Sexpr{round(KIs["x2B", 1], 4)}$ bis $\Sexpr{round(KIs["x2B", 2], 4)}$ den wahren, unbekannten Wert der Änderung des Mittelwerts von $y$ in der Population, wenn \texttt{B} statt \texttt{A} beobachtet wird.
\end{itemize}
