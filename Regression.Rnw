...
\item Zeile \texttt{x1}: Geschätzte Steigung $\hat{\beta}_{x1}=\Sexpr{round(coef(erglm)[2],5)}$. Modellierte Änderung des Mittelwerts von $y$, wenn $x1$ eine Einehit größer beobachtet wird.
