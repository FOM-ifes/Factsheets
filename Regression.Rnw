...
\title{Übericht: Lineare Regression}
...
\item $\beta_1$: Steigung, d.h. Änderung des Mittelwerts von $y$, wenn $x$ eine Einheit größer 
\end{itemize}


Kleinste Quadrate Kriterium zur Schätzung von Achsenabschnitt und Steigung mit Hilfe einer Stichprobe: $\hat{\beta}_0$ und $\hat{\beta}_1$ so, dass die Quadratsumme der Residuen $\sum \hat{\epsilon}^2_i$ mit $\hat{\epsilon}_i = y_i - (\hat{\beta}_0 + \hat{\beta}_1 x_i)$ minmal ist. Ergebnis:
\begin{itemize}
...
Tabelle \texttt{Coefficients} - Spalte \texttt{Estimate} gibt die geschätzten Koeffizienten des Modells an:
\begin{itemize}
...
\item Zeile \texttt{x1}: Geschätzter Steigung $\hat{\beta}_{x1}=\Sexpr{round(coef(erglm)[2],5)}$. Modellierte Änderung des Mittelwert von $y$, wenn $x1$ eine Einehit größer beobachtet wird.
\item Zeile \texttt{x2B}: Geschätzte Änderung $\hat{\beta}_{x2B}=\Sexpr{round(coef(erglm)[3],5)}$. Modellierte Änderung des Mittelwert von $y$, wenn $x2=B$ (anstelle von $x2=A$) beobachtet wird.
\end{itemize}
...
Tabelle \texttt{Coefficients} - Spalte \texttt{Pr(>|t|)} gibt die mit Hilfe mathematicher Statistik bestimmten p-Wert für die jeweilige Nullhypothese $H_0: \beta_j=0$ an.
\begin{itemize}
\item Zeile \texttt{(Intercept)}: p-Wert $\Sexpr{ifelse(summary(erglm)$coefficients[,4]<2e-16,"<2e-16",round(summary(erglm)$coefficients[,4],5))[1]}$. Geschätzter Achsenabschnitt statistisch erkennbar von $0$ verschieden. (Statistisch signifikant zum Niveau $\alpha=5\,\%$.)
