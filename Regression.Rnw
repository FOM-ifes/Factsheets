...
\item Zeile \texttt{x1}: Geschätzte Steigung $\hat{\beta}_{x1}=\Sexpr{round(coef(erglm)[2],5)}$. Modellierte Änderung des Mittelwert von $y$, wenn $x1$ eine Einehit größer beobachtet wird.
...
Tabelle \texttt{Coefficients} -- Spalte \texttt{Pr(>|t|)} gibt die mit Hilfe mathematischer Statistik bestimmten p-Wert für die jeweilige Nullhypothese $H_0: \beta_j=0$ an.
\begin{itemize}
