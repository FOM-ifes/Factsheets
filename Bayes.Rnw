...
\item  \textbf{Posteriori-Verteilung} $Pr(H\mid E)$: Wahrscheinlichkeitsverteilung von $H$, \emph{nachdem} wir unsere Daten haben.
\end{itemize}
...
\item \textbf{Posteriori-Verteilung}: Beta-Verteilung mit Parametern: $\alpha_{post}, \beta_{post}$:
    $$
    \pi\mid_{(Y=y)} \sim Beta(\alpha_{post}, \beta_{post})
    $$ mit
    $$
