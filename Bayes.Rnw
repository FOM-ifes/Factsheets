...
\title{Übersicht: Bayessche Statistik}
...
$$
\overbrace{Pr(H \mid E )}^{\text{Epistemisch}} = Pr( H ) \frac{\overbrace{Pr(E \mid H)}^{\text{Aleatorisch}}} {Pr(E)}
$$
\end{itemize}
...
$$
\overbrace{Pr(H \mid E)}^{\text{Posterior}} \propto \overbrace{Pr(H)}^{\text{Prior}} \cdot \overbrace{Pr(E \mid H)}^{\text{Likelihood}}
$$

\begin{itemize}
\item \textbf{Prior-Verteilung} $Pr(H)$: Wahrscheinlichkeitsverteilung von $H$, \emph{bevor} wir unsere Daten haben.
\item \textbf{Likelihood} $Pr(E\mid H)$: \emph{Mutmaßlichkeit} von $E$ bei gegebenem $H$.
\item  \textbf{Posterior-Verteilung} $Pr(H\mid E)$: Wahrscheinlichkeitsverteilung von $H$, \emph{nachdem} wir unsere Daten haben.
\end{itemize}

\section{Beta-Binomial-Modell}

\begin{itemize}
\item \textbf{Prior-Verteilung}: Grundlage Beta-Verteilung mit Parametern: $\alpha_{prior}, \beta_{prior}$:
    $$
...
\item \textbf{Likelihood}: Grundlage Binomialverteilung mit Parametern $n$ und $\pi$: 
    ...
    Y \sim Bin(n, \pi)
    $$
\item \textbf{Posterior-Verteilung}: Beta-Verteilung mit Parametern: $\alpha_{post}, \beta_{post}$:
    $$
    \pi\mid (Y=y) \sim Beta(\alpha_{post}, \beta_{post})
    $$ mit
    $$
...
$n \in \{0,1,2,\ldots\}$ und $\pi \in [0,1]$ bestimmen die Form der Verteilung.
$n$ ist die Anzahl der Versuche und $\pi$ die Erfolgswahrscheinlichkeit eines Versuches.
