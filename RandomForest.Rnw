...
Die Wichtigkeit der Variablen (interessant z.\,B. im Zusammenhang mit linearen oder logistischen Regressionsmodellen) kann mit \texttt{importance(mod)} oder \texttt{varImp(mod)} aus dem Paket \texttt{caret} ausgegeben werden.
