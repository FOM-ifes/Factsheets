...
\begin{itemize}
\item Beobachtungen, Merkmalsträger, auf der die Daten vorliegen, bspw. einzelne Menschen, Unternehmen, Länder: häufig in den Zeilen.
...
\item \textbf{Randomisiertes Experiment}: Der Wert der unabhängigen Variable(n) ("$x$") wird zufällig zugeordnet, manipuliert und die Variation der abhängigen Variable ("$y$") analysisert. 
\end{itemize}
