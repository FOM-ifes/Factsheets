...
\title{Grundlagen Quantitativer Datenanalyse}
...
\begin{itemize}
\item \textbf{Zufallsstichprobe}: Jede Beobachtung die gleiche Wahrscheinlichkeit, Teil der Stichprobe zu sein.
...
\item \textbf{Randomisierts Experiment}:  wird der Wert der unabhängigen Variable(n) ("$x$") zufällig zugeordnet manipuliert und die Variation der abhängigen Variable ("$y$") analysisert. 
\end{itemize}
...
\begin{table}[!htb]
\begin{tabularx}{\columnwidth}{X|XX}
