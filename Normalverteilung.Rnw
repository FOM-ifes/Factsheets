...
\item Die bedingte Wahrscheinlichkeit $Pr(A|B)$ beschreibt die Wahrscheinlichkeit von $A$, gegeben $B$, d.h., $B$ ist eingetreten. Es gilt für $Pr(B)>0$: $Pr(A|B) = \frac{Pr(A \text{ und } B)}{Pr(B)}$.
\item Zwei Ereignisse sind unabhängig, wenn gilt: $Pr(A \text{ und }B)=Pr(A)\cdot Pr(B)$.
\item Für zwei Ereignisse $A$ und $B$ gilt: $Pr(A \text{ oder }B) = Pr(A) + Pr(B) - Pr(A \text{ und }B)$.
\item Für die Wahrscheinlichkeit des Gegenteils von $A$, $A^G$, gilt: $Pr(A^G) = 1 - Pr(A)$.
\item Satz von der totalen Wahrscheinlichkeit: $Pr(A) = Pr(A|B) \cdot Pr(B) + Pr(A|B^G) \cdot Pr(B^G)$.
\item Satz von Bayes: $Pr(B|A)=\frac{Pr(A|B) \cdot Pr(B)}{Pr(A)}$.
\end{itemize}
...
<<fig.align="center", out.width="0.45\\linewidth", fig.asp = 0.6, echo=FALSE, message = FALSE>>=
library(mosaic)
library(latex2exp)
...
\textbf{68-95-99{.}7\%-Regel:} Bei Normalverteilung
\begin{itemize}
